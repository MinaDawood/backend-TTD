...
describe('GET /users/:username', () => {
  ...
  it('send the correct res when is err', async () => {
  ...
  it('returns res when user not found', async () => {
    const stub = sinon.stub(db, 'getUserByUsername').resolves(null);

    await request(app).get('/users/noo').expect(404);
